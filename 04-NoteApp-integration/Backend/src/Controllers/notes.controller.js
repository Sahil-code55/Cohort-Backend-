 ...
 const newNote = await NotesModel.create({
  ...
  return  res.status(201).json({
    ...
    console.log("Error in creation ",error)
     return res.status(500).json({ message: "Error in creating the note" })
 }
}
...
const getAllNotesController =  async(req,res)=>{
    ...
    return res.status(200).json({
        ...
        return res.status(500).json({ message: "Error in fetching the notes" })
    }
}
...
let updatedSingleNote = await NotesModel.findByIdAndUpdate(id,data,{new:true});

return res.status(200).json({
    message :"single note update Successfully",
    data : updatedSingleNote
        ...
        return res.status(500).json({message: "Error in updating the single note with id "+id})
    }
}
